...
def fill_missing_bets(
    *,
    ...
    candidates = _local_candidates(config, state, current_time, lookback)
    _log(verbose, f"event_id={config.event_id}")
    _log(verbose, f"mode=autobet dry_run={dry_run} now={current_time.isoformat()}")
    ...
    *,
    ...
    for match in candidates:
        ...
        if not missing_users:
            processed += 1
            if not dry_run:
                _mark_processed(state, config.event_id, match.match_id, now, 0)
        ...
        processed += 1
        if not dry_run:
            _mark_processed(state, config.event_id, match.match_id, now, len(missing_users))
    ...
    return AutoBetResult(
        ...
        generated_bets=len(generated_rows) if dry_run else inserted,
...
def _local_candidates(
    ...
    due_after = now - lookback
    candidates: list[MatchConfig] = []
    for match in config.matches:
        kickoff = _to_utc(match.kickoff_utc)
        if kickoff > now or kickoff < due_after:
            continue
...
def main() -> None:
    ...
    parser.add_argument("--lookback-minutes", type=int, default=30, help="Only process matches that started within this window.")
