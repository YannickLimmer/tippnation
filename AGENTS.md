...
- `data/events/world_cup_2026.json` now contains explicit group-stage fixtures, corrected Group G/H assignments, venues, and API-FOOTBALL event metadata. Knockout placeholders are still generated by `tippnation/config.py` until those fixtures are known.
...
- `data/events/international_friendlies_trial_2026.json` is a temporary June 5-7, 2026 deployment trial event generated from Betfair `Friendlies International` (`competition_id=12205166`). The trial was successful; `tippnation/config.py` now points `DEFAULT_EVENT_CONFIG` at `world_cup_2026.json`.
...
- Use `python -m tippnation.autobet_cli` locally on a 5-minute schedule to generate missed bets for newly started matches. It uses `data/autobet_state.json`, requires locked market score probabilities, inserts only missing active-player bets, and marks them with `bets.auto_generated = 1`.
